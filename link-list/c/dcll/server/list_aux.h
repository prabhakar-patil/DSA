...
node_t *get_node (data_t d); 	/* creates one node dynamically in memory, copies data in data field
				and returns base address of node (struct node)declarations */
	
bool_t g_insert (node_t *beg, node_t *mid, node_t *end); 	/* generic insert mid inbetween beg and end node, 
								used by insert_*() interface routines */
bool_t g_delete (node_t *del_node);	/*generic delete, used by del_*() insterface routines */
